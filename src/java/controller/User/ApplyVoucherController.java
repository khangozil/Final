...
public class ApplyVoucherController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Voucher voucher = dao.getVoucherByCode(voucherCode);
        
        if (voucher != null) {
            cart.applyVoucher(voucher);
            session.setAttribute("cart", cart);
            request.setAttribute("messSuccess", "Thêm voucher thành công!");
            request.getRequestDispatcher("User/Cart.jsp").forward(request, response);
        } else {
            request.setAttribute("mess", "Voucher không tồn tại");
            request.getRequestDispatcher("User/Cart.jsp").forward(request, response);
        }
