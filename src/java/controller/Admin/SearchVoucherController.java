...
public class SearchVoucherController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //by code
        if(option.equals("1")){
            Voucher vou = dao.searchVouchersByCode(textSearch);
            request.setAttribute("vou", vou);
        }
        else if(option.equals("2")){
            List<Voucher> list = dao.searchVouchersByRate(textSearch);
            request.setAttribute("listV", list);
        }
        else{
            request.setAttribute("listV", "");
        }
